...
class ReactAverage{
	public static function reactAverage_callback(  ): void {
		...
		if(is_user_logged_in()){
			...
			$average_react_list = get_user_meta($user_id , '_react_average_post', true);
			...
			if (in_array($post_id , $average_react_posts)) {
				...
				$average_react = array_diff( $average_react_list, [$post_id ] );
				...
				if (empty($average_react_list)) {
					update_user_meta($user_id , '_react_average_post', array($post_id ));
				} else {
					$author_arr = (is_array($average_react_list)) ? $average_react_list : array();
